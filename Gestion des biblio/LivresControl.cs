...
namespace Gestion_des_biblio
{
    ...
    public partial class LivresControl : UserControl
    {
        int id ;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand com = new SqlCommand("insert into dbo.Livres( Titre , Auteur ,Editeur , Stock , D_Ajout) values ( '"+textBox1.Text+"', '"+textBox2.Text+"', '"+textBox3.Text+ "', '" + numericUpDown1.Value + "', '" + DateTime.Parse(dateTimePicker1.Text) + "'  )", con);
                com.ExecuteNonQuery();
                con.Close();
        ...
        private void LivresControl_Load(object sender, EventArgs e)
        {
            LoadAllRecords();
       //     Timer timer = new Timer();
        ...
        void LoadAllRecords()
        {
            SqlCommand com = new SqlCommand("select * from  Livres ", con);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                if (MessageBox.Show("Are you confirm to delete ? ", "Delete", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    // textBox1.Text = row.Cells[1].Value.ToString();
                    //textBox2.Text = row.Cells[2].Value.ToString();
                    SqlCommand com = new SqlCommand("delete from dbo.Livres where Id = '" + row.Cells[0].Value.ToString() + "'", con);
                    com.ExecuteNonQuery();
                    con.Close();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand com = new SqlCommand("update dbo.Livres set  Titre =  '" + textBox1.Text + "' , Auteur = '" + textBox2.Text + "' , Editeur =  '" + textBox3.Text + "' , Stock = '" + numericUpDown1.Value + "' , D_Ajout = '" + DateTime.Parse(dateTimePicker1.Text) + "' where Id = ' " + id + "'", con);
                com.ExecuteNonQuery();
                con.Close();
